 ...
 * @author Kris
 * 
 *         This class provides synchronization between a digital signal
 *         processor and speaker output.
 */
public class KJDigitalSignalSynchronizer
{
	...
	/**
	 * Default constructor creates a both the default frames per second and
	 * frame rate ratio hint calibration.
	 */
	...
	/**
	 * Default constructor creates a DSS with a specified FPS and default frame
	 * rate ratio hint calibration.
	 */
	...
	/**
	 * @param pFramePerSecond
	 *            The desired frames per second to update and registered DSPs.
	 * @param pFrameRateRatioHintCalibration
	 *            The frame rate calibration (in FPS) to have a DSP simulate
	...
	/**
	 * Creates and returns a normalizer object that can be used to normalizer
	 * audio data.
	 * 
	...
	/**
	 * Returns the source data line write state. See
	 * {@link #setSourceDataLineWriteEnabled(boolean)} for details.
	 * 
	 * @return A boolean
	 */
	 ...
	 * 
	 * NOTE: If this is disabled, reliable buffer synchronization is NOT
	 * possible between the SDL and the DSS. Therefore it is only possible to
	 * write less than the SDL buffer size at a time to the writeAudioData()
	 * method. This usually works fine for MP3 encoded files as they are decoded
	 * into frames of about 4608 bytes. If an entire WAV file is loaded into
	 * memory ( 128kb or greater ) this feature must be enabled in order to
	 * write the entire data buffer to the DSS at once. If disabled, it is also
	 * necessary to write to the SDL manually (like KJ DSP v1.1)
	 * 
	 * @param A
	 *            boolean
	 */
	...
	public synchronized void start( SourceDataLine pSdl )
	{
		...
		if (synchronizer == null)
		{
			...
			// -- Allocate a little more memory than the SDL buffer to prevent
			// buffer overlapping.
	...
	/**
	 * Writes the entire specified buffer to the monitored source data line an
	 * any registered DSPs.
	 * 
	 * @param pAudioData
	 *            Data to write.
	 ...
	 *            Data to write.
	 * @param pOffset
	 *            Offset to start reading from the buffer.
	 * @param pLength
	 *            The length from the specified offset to read.
	 */
	...
	public class Context
	{
		...
		/**
		 * Returns a normalized sample from the DSS data buffer. This method can
		 * be used to make a DSP based on KJ DSP v1.1 work properly again.
		 * 
		...
		/**
		 * Returns the data buffer offset to start reading from. Please note
		 * that the offset + length can be beyond the buffere length. This
		 * simply means, the rest of data sample has rolled over to the
		 * beginning of the data buffer. See the Normalizer inner class for an
		 * example.
		 * 
		 * @return int
		 */
		...
		public float[][] normalize( byte[] pData, int pPosition, int pLength )
		{
			...
			for (int sp = 0; sp < sampleSize; sp++)
			{
				...
				for (int ch = 0; ch < wChannels; ch++)
				{
					...
					// long sm = ( pData[ pPosition + cdp ] ); // & 0xFF ) -
					// 128;
					...
					// System.out.println( "SP[" + sp + "] " + channels[ ch ][
					// sp ] );
			...
			// }
			//						
			// channels[ 0 ][ a ] = (float)( ( ( pData[ c ] & 0xFF ) - 128 ) /
			// 128.0f );
			//						
			...
			// }
			//						
			// channels[ 0 ][ a ] = (float)( ( (int)pData[ c + 1 ] << 8 ) +
			// pData[ c ] ) / 32768.0f;
			// //right[ a ] = left[ a ];
			//						
			// // System.out.print( channels[ 0 ][ a ] + " " );
			...
			// channels[ 0 ][ a ] = (float)( ( ( pData[ c ] & 0xFF ) - 128 ) /
			// 128.0f );
			// channels[ 1 ][ a ] = (float)( ( ( pData[ c + 1 ] & 0xFF ) - 128 )
			// / 128.0f );
			//						
			// }
			//					
			...
			// channels[ 0 ][ a ] = (float)( ( (int)pData[ c + 1 ] << 8 ) +
			// pData[ c ] ) / 32768.0f;
			// channels[ 1 ][ a ] = (float)( ( (int)pData[ c + 3 ] << 8 ) +
			// pData[ c + 2 ] ) / 32768.0f;
			//						
			// }
			//					
		...
		public void run( )
		{
			...
			while (active)
			{
				...
				try
				{
					...
					// -- Calculate the frame rate ratio hint. This value can be
					// used by
					// animated DSP's to fast forward animation frames to make
					// up for
